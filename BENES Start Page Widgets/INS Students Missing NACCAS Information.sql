...
-- Update: 7/27/2018 by Kelly MJ
    -- Added filter to only display students who were in an instructor's (current user's) class

select  st.idnumber ID_No,
          CONCAT('<a href="https://benes.orbund.com/einstein-freshair/addedit_form_instance.jsp?userId=', CAST(st.studentId AS CHAR), '&userType=1&formId=13">', st.firstName, ' ', st.lastName, '</a>') AS       -- links to Placement Verification Log Form
          'Student Name',
