...
-- 2019-05-31 Kelly MJ - excluded LOA students, added check for isActive clockPunches IN (1, 2)
...
JOIN ClassStudentReltn CSR ON SDT.studentId = CSR.studentId
JOIN Classes CLS ON CSR.classId = CLS.classId
LEFT OUTER JOIN
	(SELECT
		idNumber,
		userId,
	...
	WHERE <ADMINID>
	  AND isActive IN (1, 2)
	  AND CURDATE() = DATE(punchTime)
	GROUP BY userId) AS IDT ON SDT.studentId = IDT.userId

INNER JOIN Programmes P ON R.programmeId = P.programmeId
