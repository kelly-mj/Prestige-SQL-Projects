...
SELECT t1.Name
    ...
    , (SELECT MAX(fieldValue) FROM ProfileFieldValues WHERE userId = t1.studentId AND fieldName = 'PROGRAM_HOURS_SCHEDULED') AS 'Scheduled Hours'
    , (SELECT MAX(fieldValue) FROM ProfileFieldValues WHERE userId = t1.studentId AND fieldName = 'PROGRAM_HOURS_ATTENDED') AS 'Actual Hours'
