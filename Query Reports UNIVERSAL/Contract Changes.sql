...
SELECT t1.Name
	, DATE(t1.RA1_dateMax) AS 'Updated'
...
FROM (
	SELECT CONCAT('<a target="_blank" href="admin_view_student.jsp?studentid=', CAST(S.studentId AS CHAR), '">', S.lastName, ', ', S.firstName, '</a>') AS Name
		...
		, (SELECT MAX(lastUpdateDtTm)
            ...
            WHERE registrationId = R.registrationId AND lastUpdateDtTm < RA1_max.RA1_dateMax  AND regStatus IN (1, 12)) AS RA2_dateMax
        , RA1.*
        , RA1_max.RA1_dateMax
	...
	INNER JOIN (SELECT registrationId, MAX(lastUpdateDtTm) AS RA1_dateMax
				FROM Registrations_Audit
                WHERE isActive = 1 AND regStatus IN (1, 12) GROUP BY registrationId) RA1_max
    ...
    WHERE S.isActive = 1
	  AND CASE '[?Last Name{A-H|A-H|I-P|I-P|Q-Z|Q-Z}]'
			WHEN 'A-H' THEN SUBSTR(S.lastName, 1, 1) >= 'a' AND SUBSTR(S.lastName, 1, 1) <= 'h'
            WHEN 'I-P' THEN SUBSTR(S.lastName, 1, 1) >= 'i' AND SUBSTR(S.lastName, 1, 1) <= 'p'
            WHEN 'Q-Z' THEN SUBSTR(S.lastName, 1, 1) >= 'q' AND SUBSTR(S.lastName, 1, 1) <= 'z' END
	  AND IF('[?Campus Select (leave blank to select all)]' = ''
		, RA1.studentCampus <> 'delicious_kielbasa_sausage'
		, ((RA1.studentCampus = '[?Campus Select (leave blank to select all)]')
		    OR (RA1.studentCampus = (SELECT MAX(campusCode) FROM Campuses WHERE LOWER(campusName) = LOWER('[?Campus Select (leave blank to select all)]')) )) )
	  AND S.<ADMINID>
	) t1
