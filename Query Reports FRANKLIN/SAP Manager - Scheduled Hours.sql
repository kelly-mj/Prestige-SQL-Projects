	...
	FROM (
		SELECT CONCAT('<a target="_blank" href="admin_view_student.jsp?studentid=', CAST(S.studentId AS CHAR), '">', S.lastName, ', ', S.firstName, '</a>') AS Name
				...
				LEFT JOIN (SELECT registrationId
							, CASE programmeId WHEN 7 THEN 459		-- cosmetology
											   WHEN 9 THEN 382.5	-- aesthetics
											   ELSE MAX(payPeriodHours) END AS period1
					FROM PayPeriodDates WHERE payPeriodNo = 1 GROUP BY registrationId) PP1
					ON PP1.registrationId = R.registrationId

				LEFT JOIN (SELECT registrationId
							, CASE programmeId WHEN 7 THEN 918		-- cosmetology
											   WHEN 9 THEN 765		-- aesthetics
											   ELSE MAX(payPeriodHours) END AS period2
					FROM PayPeriodDates WHERE payPeriodNo = 2 GROUP BY registrationId) PP2
					ON PP2.registrationId = R.registrationId

                LEFT JOIN (SELECT registrationId
							, CASE programmeId WHEN 7 THEN 1224		-- cosmetology
									   		   ELSE MAX(payPeriodHours) END AS period3
					FROM PayPeriodDates WHERE payPeriodNo = 3 GROUP BY registrationId) PP3
